...
SPEI_daily <- function(wd_data, 
  ...
  # Basic checks
  # ---------------------------------------------------------------------------
  if (!all(c("Y", "M", "D", "wd") %in% names(wd_data))) {
    stop("wd_data must contain columns: Y, M, D, and wd")
  }
  
  wd_data$date <- as.Date(paste(wd_data$Y, wd_data$M, wd_data$D, sep = "-"))
  
  if (any(is.na(wd_data$date))) {
    stop("Some dates could not be converted. Please check Y, M, and D columns.")
  }
  
  # Remove Feb 29 because this function uses a 365-day calendar
  wd_data <- wd_data[format(wd_data$date, "%m-%d") != "02-29", ]
  
  # Sort by date
  wd_data <- wd_data[order(wd_data$date), ]
  
  # Optional safety warning
  if (nrow(wd_data) %% 365 != 0) {
    warning("Number of rows is not a multiple of 365 after removing Feb 29. Check whether all years are complete.")
  }
  ...
  # ---------------------------------------------------------------------------
  # Create daily time series with 365-day frequency
  # ---------------------------------------------------------------------------
  first_year <- wd_data$Y[1]
  ...
  for (s in 1:m) {
    ...
    if (scale > 1) {
      wgt <- SPEI::kern(scale, kernel$type, kernel$shift)
    ...
    # Loop through day-of-year classes
    for (c in 1:fr) {
      ...
      if (is.null(params)) {
        ...
        if (distribution == "log-Logistic" && fit == "max-lik") {
          f_params <- SPEI::parglo.maxlik(month, f_params)$para
        }
  ...
  z <- list(
    ...
    kernel = list(
      type = kernel$type,
      shift = kernel$shift,
      values = SPEI::kern(scale, kernel$type, kernel$shift)
    ),
  ...
  # ---------------------------------------------------------------------------
  # Important:
  # If x = TRUE, return diagnostic list.
  # If x = FALSE, return normal data frame used in your main loop.
  # ---------------------------------------------------------------------------
  if (isTRUE(x)) {
    z$wd_data <- wd_data
    return(z)
...
for (ij in (lower_limit:upper_limit)){
  ...
  for (j in 0: (ncol(SPEI_temp) - ij)) {
    ...
    save_trick <- nrow(temp_rwl_subset)
  }
...
if (isTRUE(remove_insignificant)) {
  
  melted$value <- ifelse(
    abs(melted$value) < critical_r(save_trick, 0.05),
    NA,
    melted$value
  )
